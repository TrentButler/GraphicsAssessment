...
void Mesh::create_buffers() 
{
	...
	glBufferData(GL_ARRAY_BUFFER, this->vertex_count * sizeof(Vertex), glm::value_ptr(this->_vertices), GL_STATIC_DRAW); //SEND OPENGL THE VBO'S INFORMATION

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, this->_ibo); //BIND THE IBO
	glBufferData(GL_ELEMENT_ARRAY_BUFFER,
		this->index_count * sizeof(unsigned int), glm::value_ptr(this->_indices), GL_STATIC_DRAW); //SEND OPENGL THE IBO'S INFORMATION
